...
|**usysesejazo avapo ilyko.**|Nearly all of them have fallen, contrary to my preference.
...
From these examples, we can explicitly see the differences in meaning when particles are attached to root tags mid-concept. From here, we derive another principle of Tokawaje: *when particles alter the meaning of words they are attached to, they only affect tags attached to the current word, not to the entire concept*. If we only wish to place emphasis on a particular aspect of a concept, an aspect that is critical to the description of that concept and therefore placed towards the front of its total tag-list, then we will need to isolate that tag within its own word in order for it to be targeted exclusively by a particle. If we only multiple tags to be affected, but not have subsequent tags included, then we simply need to merge those tags into their own word before tagging them all with a particle. We'll address more examples of this scenario in the next section.
